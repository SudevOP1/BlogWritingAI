...
async def generate_blog(
    ...
    try:
        # Get client IP address, fallback to request.client.host or 127.0.0.1
        ...
        # get the number of blogs generated by the ip address in the last 24 hours
        ...
        if count >= 3:
            latest_blog = await db.blogs.find_one(
                {"ip_address": ip_address},
                sort=[("created_at", -1)],
            )
            ...
            if latest_blog:
                remaining_time = timedelta(days=1) - (
                    datetime.utcnow() - latest_blog.get("created_at")
                )
            else:
                remaining_time = timedelta(days=1)
            ...
            # if remaining seconds is less than 0, set it to 0
            if remaining_seconds < 0:
                remaining_seconds = 0

            return {
                "success": False,
                "error": f"Rate limit exceeded. Please try again in {remaining_seconds} seconds",
