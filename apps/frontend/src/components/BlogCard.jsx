...
function BlogCard({ blog }) {
  ...
  return (
    <Link
      to={`/blog/${blog.id}`}
      className="flex flex-col gap-4 p-6 justify-between group bg-surface rounded-xl border border-slate-800
        hover:border-primary/40 transition hover:shadow-lg hover:shadow-primary/5 overflow-hidden h-50"
    >
      {/* tag */}
      ...
      <div className="flex items-center justify-between text-xs text-slate-500 pt-1 border-t border-slate-800/80">
        ...
        <div className="relative min-w-24 h-5">
          <div
            className="absolute inset-0 flex items-center justify-end gap-3 transition ease-out
          ...
          <div
            className="absolute inset-0 flex items-center justify-end transition ease-out
